...
namespace stickynav_planning
{
    ...
    OnlinePlanner::OnlinePlanner()
        : running_(false), planning_(false)
    {
        // Setup params
    ...
    void OnlinePlanner::setupFromParamMap(Module::ParamMap *param_map)
    {
        ...
        if (build_modules_on_init)
        {
            ...
            trajectory_point.position = Eigen::Vector3d(0, 0, 0);
    ...
    void OnlinePlanner::initializePlanning()
    {
        ...
        trajectory_point.position = target_position_;
    ...
    void OnlinePlanner::verifyTree(TrajectorySegment *next_segment)
    {
        ...
        for (TrajectorySegment *segment : segments)
        {
            ...
            if (segment->parent)
            {
                ...
                const float distance = (segment->trajectory.front().position -
                                        segment->parent->trajectory.back().position)
                                           .norm();
    ...
    bool OnlinePlanner::requestNextTrajectory()
    {
        ...
        target_position_ = trajectory.back().position;
