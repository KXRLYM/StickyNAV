...
public:
    ...
    void cuboid_callback(const octomap_msgs::Octomap::ConstPtr& map_msg) {
        ...
        if (corner_msg.poses.size() != 5) {
            ...
            return;
        } else if (corner_msg.poses.size() == 5 && !explorationed_started) {
            // corners are updated but exploration hasn't started
        ...
        if (octree) // can be NULL
        {
            ...
            for (int search_idx = 0; search_idx < VP_num_steps_x * VP_num_steps_y; search_idx++) {
                ...
                int dist = std::ceil(std::sqrt((robot_pose.x - x)*(robot_pose.x - x) + (robot_pose.y - y)*(robot_pose.y - y)) / map_msg->resolution);
