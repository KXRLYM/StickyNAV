...
public:
    ...
    geometry_msgs::PoseStamped calculate_sensor_position(float x, float y, float rec_x, float rec_y) {
        ...
        poseStamped.pose = transformed_cam_ray1;

        return poseStamped;
    }

    float compute_gain(float x, float y, float rec_x, float rec_y, octomap::OcTree* octree, float FOV, int FOV_resolution, int targeted, int publish=0) {
        float dy = rec_y - y;
        float dx = rec_x - x;  
        float mag = std::sqrt(dy * dy + dx * dx);
        float dy_norm = (mag != 0.0) ? (dy / mag) : 0.0;
    ...
    void turn() {
        ...
        float angular_velocity = 0.5;
        ros::Rate rate(10);
        float duration(6.28318530 /  (2*angular_velocity));
        ros::Time start_time = ros::Time::now();
        while (ros::Time::now() - start_time < ros::Duration(duration)) {
            command_vel.angular.z = angular_velocity;
            pub_cmd_vel.publish(command_vel);
    ...
    void build_tree(octomap::OcTree* unknown_octree, float map_min_x, float map_min_y, float map_max_x, float map_max_y,
        ...
        while (max_sample > num_sample) {
            ...
            //if (targeted) {
            //    V_rand = kd_tree_->randomSample(map_min_x, map_max_x,
            //                                map_min_y, map_max_y);
            //}
            ...
            if (nearest_node == nullptr) {
                ...
                if (targeted && checkConservativeCollision(unknown_octree, newNode->position.x, newNode->position.y)) {
                    continue;
                
                }

                if (targeted && checkLinearCollision(unknown_octree, nearest_node->position, newNodePosition, 3, 1)) {
                    continue;
                }

                if (checkLinearCollision(unknown_octree, nearest_node->position, newNodePosition, 3, 1)) {
                    continue;
                } else {
                    if (checkConservativeCollision(unknown_octree, newNode->position.x, newNode->position.y)) {
                    ...
                    for (const auto& neighbour : neighbours) {
                        ...
                        if (neighbour->cost + temp_dist_ < temp_cost_ && !checkLinearCollision(unknown_octree, neighbour->position, newNodePosition, 4)) {
                    ...
                    if (targeted) {
                        ...
                        heading.x = heading_x - newNode->position.x;
                        heading.y = heading_y - newNode->position.y;
                    ...
                    float gain = compute_gain(newNode->position.x, newNode->position.y, heading.x, heading.y, unknown_octree, FOV, FOV_resolution, targeted);
                    ...
                    for (const auto& neighbour : neighbours) {
                        double temp_dist_ = std::sqrt(std::pow(newNodePosition.x - neighbour->position.x, 2) + std::pow(newNodePosition.y - neighbour->position.y, 2));
                        if (newNode->cost + temp_dist_ < neighbour->cost && !checkLinearCollision(unknown_octree, neighbour->position, newNodePosition, 4) && !(neighbour->has_immutable_parent)) {
                            ROS_INFO("rewiring");
    ...
    void normal_callback(const octomap_msgs::Octomap::ConstPtr& map_msg) {
        ...
        if (!search_started) {
            ...
            for (int i = 0; i <= pi_res; i++) {
                ...
                if (checkObstacleCollision(unknown_octree, goal_pose.x, goal_pose.y)) {
                    continue;
                }   
                ...
                bool success = action_client.waitForResult(ros::Duration(60.0));
                break;
            }
            spin_done = true;
            return;
    ...
    void cuboid_callback(const visualization_msgs::MarkerArray::ConstPtr& search_area_msg, const octomap_msgs::Octomap::ConstPtr& map_msg) {
        ...
        if (!explorationed_started) {
            // set the starting positon of the exploration so that the robot can go back in the end
            start_pose.position.x = robot_pos.x;
            start_pose.position.y = robot_pos.y;
        ...
        if (octree) // can be NULL
        {
            ...
            KD_RRT::KDNode::KDNodePtr node_it = best_node;
            KD_RRT::KDNode::KDNodePtr current_node_it = current_robot_node;
        ...
        // =================================================================== Generating a path to the best node
            ...
            ROS_INFO("size of the path %li", path.size());
            KD_RRT::KDNode::KDNodePtr path_node = path[0];
            int max_executable_path(3);
            int current_path(0);
            for (auto path_node : path) {
                if (current_path >= max_executable_path) {
                    break;
                }
                ...
                compute_gain(path_node->position.x, path_node->position.y, rec_centre_x, rec_centre_y, octree, FOV, FOV_resolution*2, 1, 1);
            ...
            turn();
            current_robot_node = path_node;
            path_node->cost = 0;

            // =========================================================== Terminating condition
            if (best_node->gain < 50) {
                retry++;
